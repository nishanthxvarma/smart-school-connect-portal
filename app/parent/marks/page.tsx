...
import { 
  Award, Loader2, AlertCircle, FileText
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="py-20 flex justify-center items-center gap-2">
          ...
          <span className="text-xs text-slate-400 font-semibold">Loading academic records...</span>
        </div>
      ) : error ? (
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {marks.map((mark) => (
                  <tr key={mark.id} className="hover:bg-slate-55/10">
                    <td className="p-4 font-bold text-slate-800">{mark.student.user.name}</td>
